...
- `/start` — заявка для нового пользователя или кабинет для одобренного
- `/create` — создать ссылку
- `/cancel` — закрыть анкету или текущую сессию
...
Новый пользователь отвечает на 4 вопроса анкеты. Заявка приходит всем
админам с кнопками «Принять» и «Отказать». Кабинет открывается только
после принятия. Кнопка «Мануалы» есть в главном меню; текст для неё
задаётся в админ-панели.
